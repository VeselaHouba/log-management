...
class ElastAlerter(object):
    ...
    def generate_kibana6_link(self, rule, match):
        """ Returns link to kibana6 """
        dash_url = rule.get('kibana6_dashurl')
        filter = rule.get('kibana6_filter')
        deltaminutes = rule.get('kibana6_deltaminutes',30)
        time_start = datetime.datetime.now() - datetime.timedelta(minutes=deltaminutes)
        time_start_f = time_start.strftime('%Y-%m-%dT%H:%M:%S')
        time_end = datetime.datetime.now() + datetime.timedelta(minutes=deltaminutes)
        time_end_f = time_end.strftime('%Y-%m-%dT%H:%M:%S')
        dateappend = "_g=(refreshInterval:('$$hashKey':'object:856',display:'1%20minute',pause:!f,section:2,value:60000),time:(from:'"+time_start_f+"',mode:absolute,to:'"+time_end_f+"'))"
        kb_link = "<" + dash_url +'?' + filter + '&' + dateappend + "|Kibana Dashboard>"
        return kb_link
